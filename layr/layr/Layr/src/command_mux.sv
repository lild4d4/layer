...
always_comb begin
    ...
    case(state)
        READY: begin
            if (anti_coll)
                next_active_transmission = ANTI_COLL;
            else if (select_card)
                next_active_transmission = SELECT_CARD;
            else if (do_rats)
                next_active_transmission = RATS;
            else if (select_prog)
        ...
        SEND: begin
            if (mfrc_tx_valid && mfrc_tx_ready)
                next_state = WAIT_RX;
        end
        WAIT_RX: begin
            if (mfrc_rx_valid)
...
always_ff @(posedge clk) begin
    if (rst || idle_clear) begin
        ...
        if (state == READY && next_state == SEND) begin
            ...
            case (next_active_transmission)
                ...
                SELECT_CARD: begin
                    ...
                    mfrc_tx_data <= {
                        8'h93, 8'h70,
                        uid_cl1[39:32], uid_cl1[31:24], uid_cl1[23:16], uid_cl1[15:8], uid_cl1[7:0],
                        select_crc[15:8], select_crc[7:0],
                        184'd0
                    };
                end
                RATS: begin
        ...
        if (state == SEND && mfrc_tx_valid && mfrc_tx_ready)
            mfrc_tx_valid <= 1'b0;
