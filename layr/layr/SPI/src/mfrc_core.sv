...
module mfrc_core (
  ...
  localparam [3:0] S_IDLE = 4'd0, S_SETUP = 4'd1,  // load reg_req_* based on current step
  ...
  STEP_RD_LEVEL = 4'd7, STEP_RD_FIFO = 4'd8, STEP_RD_CTRL = 4'd9, STEP_RD_ERR = 4'd10;
    ...
    if (rst) begin
      ...
      case (state)
        ...
        S_SETUP: begin
          ...
          if (step == STEP_POLL) state <= S_POLL;
          else if (step != STEP_RD_FIFO || fifo_level != 6'd0) state <= S_REG_ISSUE;
          ...
          if (reg_resp_valid) begin
            case (step)
              ...
              STEP_IDLE_CMD: begin
                ...
                if (poll_success) step <= STEP_RD_LEVEL;
                else step <= STEP_RD_ERR;
                state <= S_SETUP;
              end

              // Read steps: go to CAPTURE to latch data
              STEP_RD_LEVEL, STEP_RD_FIFO, STEP_RD_CTRL, STEP_RD_ERR: state <= S_CAPTURE;
